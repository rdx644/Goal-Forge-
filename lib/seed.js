...
function assertSeedAllowed() {
  ...
  const isExplicitlyAllowed = process.env.ALLOW_SEED === 'true';

  if (isProduction && !isExplicitlyAllowed) {
    throw new Error('Refusing to seed in production unless ALLOW_SEED=true is set');
  }
}
...
function seed() {
  ...
  if (process.env.NODE_ENV === 'production') {
    console.log('Seeded users include employee, manager, and admin demo accounts.');
  } else {
